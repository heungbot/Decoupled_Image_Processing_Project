...
프로젝트 소개 : 본 프로젝트의 목표는 이미지 파일들을 S3 버킷에 업로드 되었을 때, SNS + SQS를 이용하여 다른 작업들과 분리된 프로세스로 이미지 라벨링 자동화를 구축하는 것입니다. 이를 위해 특정 S3 bucket에 이미지를 업로드 하는 어플리케이션을 운영하는 상황과 요구사항을 가정하였습니다. 이에 따른 적합한 아키텍처를 설계하고, Terraform 모듈로 구성하는 프로젝트입니다.
